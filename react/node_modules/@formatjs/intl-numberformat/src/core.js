...
var HasOwnProperty_1 = __importDefault(require("es-abstract/2015/HasOwnProperty"));
...
function currencyDigits(c) {
    return HasOwnProperty_1.default(currencyDigitsData, c)
...
function initializeNumberFormat(nf, locales, opts) {
    ...
    var options = opts === undefined ? Object.create(null) : ToObject_1.default(opts);
    ...
    var r = ecma402_abstract_1.ResolveLocale(exports.NumberFormat.availableLocales, requestedLocales, opt, 
    // [[RelevantExtensionKeys]] slot, which is a constant
    ['nu'], localeData, exports.NumberFormat.getDefaultLocale);
    ...
    var notation = ecma402_abstract_1.GetOption(options, 'notation', 'string', ['standard', 'scientific', 'engineering', 'compact'], 'standard');
    internalSlots.notation = notation;
    ecma402_abstract_1.SetNumberFormatDigitOptions(internalSlots, options, mnfdDefault, mxfdDefault, notation);
    var compactDisplay = ecma402_abstract_1.GetOption(options, 'compactDisplay', 'string', ['short', 'long'], 'short');
    if (notation === 'compact') {
        internalSlots.compactDisplay = compactDisplay;
    }
    var useGrouping = ecma402_abstract_1.GetOption(options, 'useGrouping', 'boolean', undefined, true);
    internalSlots.useGrouping = useGrouping;
    var signDisplay = ecma402_abstract_1.GetOption(options, 'signDisplay', 'string', ['auto', 'never', 'always', 'exceptZero'], 'auto');
...
function partitionNumberPattern(numberFormat, x) {
    ...
    if (isNaN(x)) {
        ...
        var formatNumberResult = ecma402_abstract_1.FormatNumericToString(internalSlots, x);
    ...
    switch (signDisplay) {
        case 'never':
            ...
            break;
        case 'auto':
            if (SameValue_1.default(x, 0) || x > 0 || isNaN(x)) {
                sign = 0;
            }
            else {
                sign = -1;
...
ecma402_abstract_1.defineProperty(exports.NumberFormat, 'supportedLocalesOf', {
    value: function supportedLocalesOf(locales, options) {
        return ecma402_abstract_1.SupportedLocales(exports.NumberFormat.availableLocales, Intl.getCanonicalLocales(locales), options);
    },
});
...
function setNumberFormatUnitOptions(nf, options) {
    ...
    var internalSlots = get_internal_slots_1.default(nf);
    var style = ecma402_abstract_1.GetOption(options, 'style', 'string', ['decimal', 'percent', 'currency', 'unit'], 'decimal');
    internalSlots.style = style;
    var currency = ecma402_abstract_1.GetOption(options, 'currency', 'string', undefined, undefined);
    if (currency !== undefined && !ecma402_abstract_1.IsWellFormedCurrencyCode(currency)) {
        throw RangeError('Malformed currency code');
    }
    if (style === 'currency' && currency === undefined) {
        throw TypeError('currency cannot be undefined');
    }
    var currencyDisplay = ecma402_abstract_1.GetOption(options, 'currencyDisplay', 'string', ['code', 'symbol', 'narrowSymbol', 'name'], 'symbol');
    var currencySign = ecma402_abstract_1.GetOption(options, 'currencySign', 'string', ['standard', 'accounting'], 'standard');
    var unit = ecma402_abstract_1.GetOption(options, 'unit', 'string', undefined, undefined);
    if (unit !== undefined && !ecma402_abstract_1.IsWellFormedUnitIdentifier(unit)) {
        throw RangeError('Invalid unit argument for Intl.NumberFormat()');
    }
    if (style === 'unit' && unit === undefined) {
        throw TypeError('unit cannot be undefined');
    }
    var unitDisplay = ecma402_abstract_1.GetOption(options, 'unitDisplay', 'string', ['short', 'narrow', 'long'], 'short');
    if (style === 'currency') {
...
function computeExponent(numberFormat, x) {
    ...
    var magnitude = ecma402_abstract_1.getMagnitude(x);
    var exponent = computeExponentForMagnitude(numberFormat, magnitude);
    // Preserve more precision by doing multiplication when exponent is negative.
    x = exponent < 0 ? x * Math.pow(10, -exponent) : x / Math.pow(10, exponent);
    var formatNumberResult = ecma402_abstract_1.FormatNumericToString(get_internal_slots_1.default(numberFormat), x);
    if (formatNumberResult.roundedNumber === 0) {
        return [exponent, magnitude];
    }
    var newMagnitude = ecma402_abstract_1.getMagnitude(formatNumberResult.roundedNumber);
